...
(
	...
	if (opts[1] == ',') {
		m_options.add_option(m_group, std::string(1, opts[0]), opts.substr(2), desc, value, std::move(arg_help));
	} else {
		m_options.add_option(m_group, "", opts, desc, value, std::move(arg_help));
	}
